...
- **UI/UX Enhancement**: Integrated `fatih/color` for vibrant output. Implemented a strict "5 items per target" truncation for both `scan` and `clean` (Dry Run) commands to keep CLI output concise and high-performance. Removed verbose nested directory listings in Dry Run. Full untruncated logs are automatically written to `/tmp/mls-last-run.log` (overwritten per run) for detailed auditing.
